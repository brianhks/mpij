...
class StreamPipe extends Thread
		...
		try
			...
			if (m_multiThreaded)
				...
				while ((line = m_input.readLine()) != null)
					{
					if (m_prefix != null)
						m_output.write(m_prefix, 0, m_prefix.length());
	...
	public void waitForString()
		...
		{
		if (m_waitForString == null)
			throw (new MPIException("waitForString called without specifying string to wait for"));
